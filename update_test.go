...
// ========================================
// Tests for detectChafaFormat
// ========================================

func TestDetectChafaFormat(t *testing.T) {
	tests := []struct {
		name           string
		termProgram    string
		term           string
		...
		},
		{
			...
			expectedFormat: "iterm",
			expectedGraphics: true,
		},
		{
			name:           "tmux (default to symbols)",
			termProgram:    "tmux",
			expectedFormat: "symbols",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if format != tt.expectedFormat {
				t.Errorf("detectChafaFormat() format = %q, want %q", format, tt.expectedFormat)
			}
			if usesGraphics != tt.expectedGraphics {
				t.Errorf("detectChafaFormat() usesGraphics = %v, want %v", usesGraphics, tt.expectedGraphics)
			}
		})
	}
}
